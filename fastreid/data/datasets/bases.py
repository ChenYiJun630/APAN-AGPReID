...
#       存储训练、查询、图库数据以及转换函数等参数
        ...
        #如果设置了 combineall，则调用 combine_all 方法合并所有数据
        if self.combineall:
            self.combine_all()
    ...
    #解析数据列表，统计行人ID数量和摄像头数量，使用集合(set)去重，确保统计的是唯一ID数量
...
class ImageDataset(Dataset):
    ...
    def show_train(self):
        #调用 parse_data 方法解析训练数据，获取行人ID数量和摄像头数量
        ...
        # tabulate it
        #使用 tabulate 库生成格式化表格，并通过 logger 输出彩色日志
        table = tabulate(
            csv_results,
            tablefmt="pipe",
            headers=headers,
    ...
    def show_test(self):
        #分别解析query和gallery数据，获取行人ID数量和摄像头数量
