...
class VisionTransformer_multiview_onebranch(nn.Module):
    ...
    def forward(self, x, camera_id=None):
        ...
        global_features = x[:, 0:1]#全局特征clstorch.Size([16, 1, 768])
        view_features = x[:, 1:2]#视觉特征torch.Size([16, 1, 768])
        local_features = x[:, 2:]#剩余特征[128,210,768]

        global_features = global_features - view_features

    
        return global_features.reshape(x.shape[0], -1, 1, 1), view_features.reshape(x.shape[0], -1, 1, 1)
        # return enh_features.reshape(x.shape[0], -1, 1, 1),global_features.reshape(x.shape[0], -1, 1, 1), view_features.reshape(x.shape[0], -1, 1, 1)
