...
class Baseline_multiview(nn.Module):
    ...
    def __init__(
        ...
        self.enhance_heads=enhance_heads
    ...
    def from_config(cls, cfg):
        ...
        enhance_heads = build_heads(cfg0)

        return {
            'backbone': backbone,
            'heads': heads,
            'view_heads': view_heads,
            'enhance_heads': enhance_heads,
    ...
    def forward(self, batched_inputs):
        ...
        #单独对global_feat进行增强
        # features,global_feats,  view_feats = self.backbone(images, camids)
   

        #多加一个feature
        enhance_feats,global_feats,view_feats = self.backbone(images, camids)
        ...
        if self.training:
            ...
            #加一个增强的features
            outputs_enhance = self.enhance_heads(enhance_feats,targets)
            losses = self.losses(outputs, outputs_global, view_outputs,outputs_enhance, targets, targets_view)
            ...
            outputs = self.heads(features)
            outputs_enhance = self.enhance_heads(enhance_feats)
            outputs = torch.cat((outputs,outputs_enhance),dim=1)
    ...
    def losses(self, outputs, outputs_global, outputs_view,outputs_enhance, gt_labels, view_labels):
        # model predictions
        # fmt: off
        ...
        enhance_pred_class_logits = outputs_enhance['pred_class_logits'].detach()
        enhance_cls_outputs = outputs_enhance['cls_outputs']
        enhancel_pred_features = outputs_enhance['features']
        # fmt: on
        ...
        if 'CrossEntropyLoss' in loss_names:
            ...
            #计算主特征的ID分类损失
            loss_dict['loss_cls_id'] = cross_entropy_loss(
                cls_outputs,#提取分类输出结果（通常是经过softmax的概率分布）
                gt_labels,
                ce_kwargs.get('eps'),
                ce_kwargs.get('alpha')
            ) * ce_kwargs.get('scale')
            ...
            loss_dict['loss_cls_enhance'] = cross_entropy_loss(
                enhance_cls_outputs,
                gt_labels,
                ce_kwargs.get('eps'),#平滑参数
                ce_kwargs.get('alpha')#类别权重
            ) * ce_kwargs.get('scale')#缩放因子
            #计算视角分类损失
            if view_id_flag:
                loss_dict['loss_cls_view'] = cross_entropy_loss(
                    view_cls_outputs,
                    view_labels,#前两个用的真实标签,这个用的是视角标签
                    ce_kwargs.get('eps'),
                    ce_kwargs.get('alpha')
                ) * ce_kwargs.get('scale') * view_lambda
        ...
        if 'TripletLoss' in loss_names:
            ...
            loss_dict['loss_triplet_enhance'] = triplet_loss(
                enhancel_pred_features,
                gt_labels,
                tri_kwargs.get('margin'),
                tri_kwargs.get('norm_feat'),
                tri_kwargs.get('hard_mining')
