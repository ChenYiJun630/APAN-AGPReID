...
if __name__ == '__main__':
    ...
    # pdf_path = os.path.join(args.output, "tsne_CARGO3.pdf")
    # png_path = os.path.join(args.output, "tsne_CARGO3.png")
    pdf_path = os.path.join(args.output, "tsne_AGReIDv22.pdf")
    png_path = os.path.join(args.output, "tsne_AGReIDv22.png")

    try:
        # PDF：紧缩边距
        plt.savefig(pdf_path, bbox_inches='tight')
        # PNG：常用高分辨率；是否紧缩边距看你需求（这里不紧缩）
        plt.savefig(png_path, dpi=300)
    except Exception as e:
        print("Save error:", e, pdf_path, png_path)
# 可选：plt.close()
